...
The 24 retained images are the immutable submitted-build-7 artifact while version 1.0 is in App Review. They are not recast as captures of the later post-publication Garden. The validator permits that distinction only for one fixed source digest and one fixed path delta, with negative controls for near matches; any further app-source change fails closed. Generate and review fresh current-source captures before the next editable App Store metadata opportunity.
