...
const exact: SubmittedBuildCaptureFreeze = {
  ...
  rationale:
    "Exact build 7 was already WAITING_FOR_REVIEW and must not be replaced, cancelled, edited, or resubmitted. The post-submission Garden is deliberately not represented by the submitted App Store screenshots; the submitted App Store listing and screenshots remained untouched. The READY_TO_SUBMIT IAP is separate and not attached, and fresh captures are required at the next editable metadata opportunity.",
};
...
assert.equal(accepts(exact, `${POST_SUBMISSION_SOURCE_REVISION.slice(0, -1)}1`), false, "a near-match revision must fail");
