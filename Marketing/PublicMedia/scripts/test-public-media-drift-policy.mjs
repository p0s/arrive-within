...
assert.equal(isExactPostPublicationMediaFreeze({ ...exact, manifest: withAttestation({ rationale: manifest.post_generation_change.rationale.replace("not current Garden proof", "current") }) }), false, "the historical-media boundary must remain explicit");
