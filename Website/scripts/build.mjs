...
function navigation(locale, activePage) {
  ...
  return `<header class="site-header">
    ...
    <nav class="navigation" aria-label="${locale === "de" ? "Hauptnavigation" : "Primary navigation"}">
      <a class="brand" href="${home}" aria-label="Arrive Within ${content.nav.home}"><span class="brand-mark" aria-hidden="true">●</span><span>Arrive Within</span></a>
      <div class="nav-links">
        ...
        <a class="language-link" href="${switchRoute}" hreflang="${counterpart}">${content.switchLabel}</a>
      </div>
    </nav>
  </header>`;
}
...
function footer(locale) {
  ...
  return `<footer class="site-footer">
    <div><a class="brand footer-brand" href="${routeFor(locale, "home")}"><span class="brand-mark" aria-hidden="true">●</span><span>Arrive Within</span></a><p>${escapeHtml(content.footer.statement)}</p></div>
  ...
  const body = `<main id="main">
    <section class="hero">
      <div class="hero-copy">
        ...
        <div class="actions"><a class="primary-action" href="#growth">${escapeHtml(home.primaryAction)}</a><a class="text-action" href="${routeFor(locale, "privacy")}">${escapeHtml(home.secondaryAction)}</a></div>
        <p class="status-note">${escapeHtml(content.footer.status)}</p>
      </div>
      <figure class="hero-visual">
        <div class="device phone"><img src="${asset(locale, "garden")}-iphone.png" width="1206" height="2622" alt="${escapeHtml(alt.gardenPhone)}"></div>
        <figcaption>${escapeHtml(home.visualNote)}</figcaption>
      </figure>
    </section>

    <section class="growth-film-section">
...
async function main() {
  assertToolchain();
  await cleanDist();
  const sourceHash = await hashTree(ROOT, new Set(["dist", "node_modules"]));
