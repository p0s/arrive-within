  ...
  en: {
    ...
    privacyPage: {
      ...
      analyticsChoice: {
        ...
        paragraphs: ["This public website counts eligible public HTML document requests at its Cloudflare edge, including cached documents, only after the static asset response is HTTP 200 with HTML content. Eligibility is limited to known public document routes. It uses no browser tracker, advertising identifier, cookie banner, or cross-site identity.", "The server sends the public path, referring origin, country from Cloudflare request metadata, and transient IP/User-Agent data to estimate aggregate sessions. The ingestion service discards the raw IP and User-Agent after processing; retained website data follows a 13-month policy, with encrypted operational backup copies expiring within 30 days after live removal. This is scoped to this website and separate from the Arrive Within app, which remains local-first and has no analytics.", "Use either control below at any time. The preference cookie is host-only, Secure, HttpOnly, and used only to honor this choice. DNT and Global Privacy Control are always honored."],
