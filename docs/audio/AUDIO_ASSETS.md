...
On 2026-08-13 the owner approved the hash-bound `Still Air` ambience and `bell-v1` family as the shipping sound direction. Physical iPhone use of build 9 then proved that the selected bell masters peaked around -41 dBFS and were effectively inaudible. Build 11 made the closing bell audible, but physical use still found the opening cue too easy to miss and the ambience inaudible at its default 30% mix. Measurement confirmed that the old ambience averaged -36 dBFS before the runtime volume reduction and concentrated most energy below a phone speaker's useful range.

The deterministic opening and closing synthesis now receives 33 dB and 29 dB of output gain respectively before a headroom-safe limiter. `Still Air` preserves its low warmth while adding 220–440 Hz harmonics that a built-in speaker can reproduce, then receives 14 dB of output gain before limiting. Automated validation requires audible, headroom-safe loudness bands for all three procedural layers. The non-shipping sound-lab alternatives remain excluded. The exact replacement candidate still requires review through built-in speaker, wired output where available, Bluetooth, lock, interruption, and route-loss scenarios. The ambience loop seam, perceptual loudness, fatigue, haptic relationship, and balance with approved narration also remain explicit device gates.
