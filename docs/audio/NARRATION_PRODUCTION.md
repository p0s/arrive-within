...
The earlier v2/v3 fragment planners are superseded by the no-synthetic-intra-sentence contract in ADR-0006 and the v4 whole-utterance implementation. One authored spoken sentence is now one model call and one VTT cue. Punctuation and lexical order remain unchanged, generated blank edges are trimmed, and no zero-filled audio is inserted inside speech; natural punctuation timing is preserved in the model output. Catalog-duration remainder is allocated only over authored `[Pause …]` directives. A 450 ms internal near-digital-silence limit fails a generated unit and the shared source gate scans every spoken VTT cue with the same threshold; sentence/paragraph gaps and authored meditation pauses remain separate. The complete 42×2 v4 batch was regenerated from lossless model output and passes the objective scan. A missing-EOS continuation gets at most two deterministic provenance-bound seed retries; all other failures remain fail-closed. Human review and public promotion are separate gates.
