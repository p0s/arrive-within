...
The capture validator binds the retained images to their original source manifest and the exact submitted build. After submission, the Garden source intentionally changed visually. Because version 1.0 build 7 was already `WAITING_FOR_REVIEW`, neither the build nor its 24 live images was replaced, cancelled, edited, or resubmitted. A narrow deterministic freeze accepts only the current fixed source digest and exact 17-path delta; unit controls reject a near-match digest, any extra path, another build, a softened mutation boundary, or a missing deferred action. The new Garden's rendered QA is separate proof, and fresh current-source App Store captures remain required at the next editable metadata opportunity.
